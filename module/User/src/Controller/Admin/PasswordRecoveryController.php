...
class PasswordRecoveryController extends BaseController
{
    ...
    public function indexAction()
    {
        ...
        if ($request->isPost()) {
            ...
            $passwordRecoveryService = $this->getServiceManager()
                ->get(PasswordRecoveryService::class);
            ...
            $this->flashMessenger()
                ->setNamespace($flashMessengerNamespace)
                ->addMessage($flashMessengerMessage);
    ...
    public function recoverPasswordAction()
    {
        ...
        if (! $passwordRecoveryToken = $entityManager->getRepository(PasswordRecoveryToken::class)->findOneBy([
            ...
            return $this->redirect()
                ->toRoute('admin-password-recovery-error');
        }

        if ($passwordRecoveryToken->getUser()
                ->getUsername() != $email) {
            $this->flashMessenger()
                ->setNamespace('error')
